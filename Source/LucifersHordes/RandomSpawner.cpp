...
void ARandomSpawner::CreateRooms()
{
...
void ARandomSpawner::SetRoomDoors()
{
...
void ARandomSpawner::SpawnRooms()
{
	...
	for (int x = 0; x < WorldSize.X*2; x++) {
		for (int y = 0; y < WorldSize.Y*2; y++) {
			...
			int targetLevel = 0;
			TArray<FName> targetRoomList = CrossRooms;
			switch (Rooms[TwotoOne(x,y)]->roomType)
			{
			case ERoomType::TRoom:
				targetRoomList = TRooms;
				break;
			case ERoomType::CrossRoad:
				targetRoomList = CrossRooms;
				break;
			case ERoomType::Straight:
				targetRoomList = StraightRooms;
				break;
			case ERoomType::Turn:
				targetRoomList = TurnRooms;
				break;
			case ERoomType::End:
				targetRoomList = EndRooms;
				break;
			case ERoomType::StartRoom:
				targetRoomList = StartRoom;
				break;
			case ERoomType::BossRoom:
				targetRoomList = BossRooms;
				break;
			default:
				targetLevel = -1;
				targetRoomList = CrossRooms;
			...
			if (targetLevel != -1) {
				targetLevel = FMath::RandRange(0, targetRoomList.Num()-1);
			}
			if (targetLevel >= targetRoomList.Num()) {
			...
			ULevelStreaming* level = UGameplayStatics::GetStreamingLevel(this, targetRoomList[targetLevel])->CreateInstance(Uniquename);
			if (level == nullptr) {
...
void ARandomSpawner::SpawnPlayer()
{
	GEngine->AddOnScreenDebugMessage(1, 50, FColor::Purple, TEXT("Spawning Player and randomizing rooms"));
	for (int x = 0; x < WorldSize.X * 2; x++) {
		for (int y = 0; y < WorldSize.Y * 2; y++) {
			if (Rooms[TwotoOne(x, y)] == nullptr) {
				continue;
...
void ARandomSpawner::SetSpecialRooms()
{
	//Set SpawnRoom
	bool bSetSpawn = false;
	for (int x = 0; x < WorldSize.X * 2 && !bSetSpawn; x++) {
		for (int y = 0; y < WorldSize.Y * 2 && !bSetSpawn; y++) {
			if (Rooms[TwotoOne(x, y)] == nullptr) {
				continue;
			}
			if (Rooms[TwotoOne(x, y)]->roomType == ERoomType::End) {
				Rooms[TwotoOne(x, y)]->roomType = ERoomType::StartRoom;
				StartLocation.X = x;
				StartLocation.Y = y;
				bSetSpawn = true;
			}
	...
	if (!bSetSpawn) {
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Blue, TEXT("Could not spawn Start room"));
	}

	//Set BossRoom
	bool bSetBoss = false;
	for (int x = (WorldSize.X * 2) - 1; x >= 0 && !bSetBoss; x--) {
		for (int y = (WorldSize.Y * 2) - 1; y >= 0 && !bSetBoss; y--) {
			if (Rooms[TwotoOne(x, y)] == nullptr) {
				continue;
			}
			if (Rooms[TwotoOne(x, y)]->roomType == ERoomType::End) {
				Rooms[TwotoOne(x, y)]->roomType = ERoomType::BossRoom;
				BossLocation.X = x;
				BossLocation.Y = y;
				bSetBoss = true;
			}
	...
	if (!bSetBoss) {
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Blue, TEXT("Could not spawn boss room"));
	}
}

void ARandomSpawner::IdRoom(int x, int y)
{
...
bool ARandomSpawner::CompareDoors(FDoors door1, FDoors door2) {
...
int ARandomSpawner::TwotoOne(int x, int y)
{
	return (y*(WorldSize.X*2))+x;
}

FVector2D ARandomSpawner::NewPosition()
{
	int x = 0, y = 0;
	FVector2D checkPos = FVector2D::ZeroVector;
	int iter = 0;
...
FVector2D ARandomSpawner::SelectiveNewPosition()
{
	int inc = 0, index = 0;
	int x = 0, y = 0;
	FVector2D checkPos = FVector2D::ZeroVector;
...
int ARandomSpawner::NumberOfNeighbors(FVector2D checkPos, TArray<FVector2D> usedLocations)
{
	int neighbors = 0;
...
void ARandomSpawner::BeginPlay()
{
	...
	//Check to see if there are more rooms requested than can fit inside the 
	NumOfRooms += Cast<UUIManagerGameInstance>(GetWorld()->GetGameInstance())->completedRuns * DifficultyScale;
	if (NumOfRooms >= (WorldSize.X * WorldSize.Y)) {
		NumOfRooms = FMath::RoundToInt(WorldSize.X * WorldSize.Y)*0.85;
	}
	CreateRooms();
	SetRoomDoors();
	SetSpecialRooms();
